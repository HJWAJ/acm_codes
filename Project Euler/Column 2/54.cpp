...
long long calc(char s[][5])
{
    ...
    for(i=0;i<5;i++)
    {
        if(s[i][0]<='9')bo[s[i][0]-'0'-1]++;
        else if(s[i][0]=='T')bo[9]++;
        else if(s[i][0]=='J')bo[10]++;
        else if(s[i][0]=='Q')bo[11]++;
        else if(s[i][0]=='K')bo[12]++;
        else bo[13]++;
    }
    for(i=0;i<4;i++)
    ...
    if(flag==4)
    {
        if(bo[13]==1 && bo[12]==1 && bo[11]==1 && bo[10]==1 && bo[9]==1)res+=score[9]*13;
        else
        {
            for(i=12;i>=5;i--)
                if(bo[i]==1 && bo[i-1]==1 && bo[i-2]==1 && bo[i-3]==1 && bo[i-4]==1)
                {
                    res+=score[8]*i;
                    break;
                }
...
int main()
{
    ...
    char s[5][5];
    score[0]=1;
    for(i=1;i<10;i++)score[i]=score[i-1]*14;
    while(scanf("%s",s[0])!=EOF)
    {
        a=b=0;
        for(i=1;i<5;i++)scanf("%s",s[i]);
        a=calc(s);
